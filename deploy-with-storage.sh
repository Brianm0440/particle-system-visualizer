...
# Deploys your application using Cloud Storage + Cloud Run
...
echo -e "${BLUE}🚀 Deploying your 3D Particle System with Cloud Storage...${NC}"

# Configuration
PROJECT_ID="cursor-468420"
BUCKET_NAME="particle-system-files-$(date +%s)"
REGION="us-central1"
...
# Upload your files
echo -e "${BLUE}📤 Uploading your application files...${NC}"
...
# Set bucket to public (since we have authentication at the Cloud Run level)
echo -e "${BLUE}🔐 Setting bucket permissions...${NC}"
gsutil iam ch allUsers:objectViewer gs://$BUCKET_NAME

# Create a simple Node.js server that serves from Cloud Storage
echo -e "${BLUE}🏗️ Creating Cloud Run service...${NC}"
cat > server-storage.js << 'EOF'
const express = require('express');
const app = express();
const PORT = process.env.PORT || 8080;

// Serve your 3D Particle System
app.get('/', (req, res) => {
    res.send(`
<!DOCTYPE html>
<html lang="en">
...
{
  "name": "particle-system-server",
  "version": "1.0.0",
  "description": "3D Particle System Server",
  "main": "server-storage.js",
  "scripts": {
    "start": "node server-storage.js"
  },
  "dependencies": {
    "express": "^4.18.2"
  },
...
# Deploy the server
echo -e "${BLUE}🚀 Deploying your 3D Particle System server...${NC}"
...
echo "   2. Run: ./deploy-with-storage.sh"
...
# Clean up
rm -f server-storage.js package.json Dockerfile
